defmodule KartVidsWeb.LeaderboardLive.Index do
  ...
  def handle_event("change_type", %{"leaderboard_type" => %{"type" => "fastest_lap"}}, socket) do
    {
      ...
      |> push_navigate(to: ~p"/locations/#{socket.assigns.location}/leaderboard")
    }
  end
  ...
  def handle_event("change_type", %{"leaderboard_type" => %{"type" => "most_races"}}, socket) do
    {
      ...
      |> push_navigate(to: ~p"/locations/#{socket.assigns.location}/leaderboard?type=most_races")
    }
  end
  ...
  # Handle all time differently
  def handle_event("search", params = %{"after_date" => %{"quick" => "9999"}}, socket) do
    beginning_date =
      Date.utc_today()
      |> Date.diff(NaiveDateTime.to_date(socket.assigns.location.inserted_at))
      |> Integer.to_string()

    handle_event("search", params |> put_in(["after_date", "quick"], beginning_date), socket)
  end
  ...
  defp quick_select_options(%Location{adult_kart_reset_on: adult_kart_reset_on, junior_kart_reset_on: junior_kart_reset_on}) do
    [
      "Last Week": 7,
      "Last Month": 30,
      "90 days": 90,
